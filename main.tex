...
Live coding is a practice of constructing and
interacting with algorithmic processes to create art.  live
coding tries to communicate algorithmic thinking, real
time composition, and networked collaborations to the
audience. 
[I still have to add things here, maybe finding new definitions]~\cite{GenerativeaRT}

\subparagraph{Key words} Algorithmic thinking, performance,
conversational methodologies, real-time composition, writing, community.
...
Live coding is a practice in which the programmer writes their own writing machine. These are open machines, supported by a community.
 ...
 but is used by programming it\cite{meaningLife}.
  ...
  on site or online, and the way in which the code is written and executed in real time. How does time or timeliness exist in this on the fly programming?
...
Algorithms are written and operated on in real time\cite{blackwell2022live}. Another key element is demonstrating to the audience that what they are witnessing is being created live, which is why the performer's screen is typically projected, revealing everything, including mistakes.
...
In June last year, I had the opportunity to teach a workshop on Tidal Cycles with the NoiseMeetup collective. 
...
From that first experience, we learned that in order to start a jam, certain rules must be established, as having more than 15 people play together on the same interface can quickly lead to chaos. This workshop also led to the creation of Toplap Linz (Transnational Organisation for the Parsimony of Live Art Programming). Founded in 2004, TOPLAP is an organisation that explores and promotes live coding. This collective functions like a rhizome: anyone with the necessary motivation and knowledge can start a Toplap group and establish a community spread all over the world.

After this, we also started organising meetings to answer questions, share knowledge and try new things. However, these meetings were not as well received, so after a few attempts, we began to wonder if our reach was limited, or if the invitations were ambiguous or unappealing.
...
This experience makes the idea of starting from scratch and programming on the fly feel even more real. Jam sessions with others greatly increase the level of spontaneity, as it's impossible to predict what each person will contribute to the performance. This is enjoyable and enables you to connect with others by creating a shared audiovisual experience. The absence of control enables you to achieve the unexpected.

\newpage

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{sheet.jpeg}
  \caption{Picture from Toplap Linz meeting, March 2025. Photo by Mascha Ilish}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{cheatsheet (1)_page-0001.jpg}
  \caption{Picture from Toplap Linz meeting, March 2025. Photo by Mascha Ilish}
\end{figure}


\subsection{Laptop Jamming}
...
what is algorithms and why is interesting to think about it.
Process as main point.
Debugging and optimization vs Live coding, errors and experimentation
(displaying algorithmic thoughts)
\newpage

\section{Some History of Live Coding}
...
\section{livecoding horizons}
This is a conclusion and a draft of thing that from this precise moment in 
the history  could be important in the future 
